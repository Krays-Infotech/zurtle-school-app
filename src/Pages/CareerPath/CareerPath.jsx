...
import getCarrerPath from "../../Redux/Reducers/Result/getCarrerPath";
...
const CareerPath = () => {
  ...
  const data = useSelector((state) => state.getResult.resultDetails);

  const fetchCarrerPath = async () => {
    try {
      const res = await dispatch(getCarrerPath(data.student)).unwrap();
    } catch (err) {
      console.log(err);
    }
  };
